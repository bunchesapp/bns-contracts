...
describe('BNSRegistry.sol', async () => {
  ...
  it('should allow ownership transfers', async () => {
    ...
    expect(await bns.setOwner(ZERO_HASH, addr1.address))
  ...
  it('should allow setting resolvers', async () => {
    ...
    expect(await bns.setResolver(ZERO_HASH, addr1.address))
  ...
  it('should allow setting the TTL', async () => {
    ...
    expect(await bns.setTTL(ZERO_HASH, 3600))
      .to.emit(bns, 'NewTTL')
  ...
  it('should allow the creation of subnodes', async () => {
    ...
    expect(await bns.setSubnodeOwner(ZERO_HASH, sha3('b'), addr1.address))
